...
router.get('/message/:index', (req, res) => {
  ...
  res.render('edit', { title: 'Edit Message', message: messages[index], index });
});

router.post('/message/:index', (req, res) => {
  const index = parseInt(req.params.index);
  if (isNaN(index) || index < 0 || index >= messages.length) {
    return res.redirect('/');
  }
  messages[index].user = req.body.userName;
  messages[index].text = req.body.messageText;
  res.redirect('/');
});
